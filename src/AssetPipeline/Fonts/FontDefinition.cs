...
public sealed class FontGenerationSettings
{
    ...
    internal void Validate()
    {
        if (EmSize is < 16 or > 256)
            throw new FontBuildException("Font generation emSize must be between 16 and 256 pixels.");
        if (DistanceRange is < 1 or > 32)
            throw new FontBuildException("Font generation distanceRange must be between 1 and 32 pixels.");
        if (Padding is < 0 or > 32)
            throw new FontBuildException("Font generation padding must be between 0 and 32 pixels.");
    }
}
