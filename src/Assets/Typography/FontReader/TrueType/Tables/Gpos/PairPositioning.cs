...
/// <summary>
/// Parses explicit and class-based GPOS Pair Adjustment subtables, retaining only the first glyph's xAdvance.
/// </summary>
internal static class PairPositioning
{
    ...
    /// <param name="candidates">The glyph indices needed by the current build.</param>
    /// <param name="adjustments">The shared pair adjustments under construction.</param>
    /// <returns><see langword="true"/> when the first-glyph ValueRecord contains xAdvance.</returns>
    ...
    )
    {
        ...
        for (var pairSetIndex = 0; pairSetIndex < pairSetCount; pairSetIndex++)
        {
            ...
            for (var pairIndex = 0; pairIndex < pairValueCount; pairIndex++)
            {
                ...
                var adjustment = ReadXAdvance(pairSetReader, valueFormat1);
                _ = ReadXAdvance(pairSetReader, valueFormat2);
    ...
    )
    {
        ...
        for (var class1 = 0; class1 < class1Count; class1++)
        {
            ...
            {
                var adjustment = ReadXAdvance(reader, valueFormat1);
                _ = ReadXAdvance(reader, valueFormat2);
                if (adjustment == 0)
                    continue;
    ...
    /// <summary>
    /// Consumes one ValueRecord and returns only its xAdvance component in font units.
    /// </summary>
    ...
    /// <returns>The signed xAdvance value, or zero when absent. All other fields are consumed and ignored.</returns>
    /// <exception cref="InvalidDataException">A selected ValueRecord is truncated.</exception>
    private static int ReadXAdvance(TrueTypeReader reader, ushort valueFormat)
    {
        var xAdvance = 0;
