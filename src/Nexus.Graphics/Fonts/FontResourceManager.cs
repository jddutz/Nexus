...
) : VulkanResourceManager<FontDefinition, FontResource>, IFontResourceManager
{
    ...
    public override FontResource CreateResource(FontDefinition definition)
    {
        ...
        // Generate atlas
        var (atlasData, glyphs, lineHeight, ascender, descender) = GenerateAtlas(
            fontData,
            definition
        );
    ...
    private (
        ...
        int Ascender,
        int Descender
    ) GenerateAtlas(byte[] fontData, FontDefinition definition)
    {
    ...
    /// </summary>
    private IGeometryResource? GenerateSharedGeometry(
        Dictionary<char, GlyphInfo> glyphs,
        string fontName
    )
    {
        ...
        foreach (var kvp in glyphs.OrderBy(g => g.Value.CharIndex))
        {
            ...
            vertices.Add(
                new Vertex<Vector2D<float>, Vector2D<float>>
                {
    ...
    private static List<char> GetCharacterRange(CharacterRange range)
    {
        return range switch
        {
            ...
            CharacterRange.Custom => throw new NotSupportedException(
                "Custom character ranges not yet implemented"
            ),
    ...
    {
        context.VulkanApi.GetImageMemoryRequirements(
            context.Device,
            image,
        ...
        var result = context.VulkanApi.AllocateMemory(
            context.Device,
            &allocInfo,
            null,
            &imageMemory
    ...
    private void UploadPixelsToImage(
        Image image,
        IntPtr pixels,
        int width,
        int height,
    ...
    )
    {
        ...
        context.VulkanApi.CmdCopyBufferToImage(
            commandBuffer,
            buffer,
            image,
            ImageLayout.TransferDstOptimal,
    ...
    protected override void DestroyResource(FontResource resource)
    {
        ...
        {
            context.VulkanApi.DestroyImageView(
                context.Device,
                resource.AtlasTexture.ImageView,
                null
            );
        }
