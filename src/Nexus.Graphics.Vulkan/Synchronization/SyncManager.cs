...
public sealed class SyncManager : ISyncManager
{
    ...
    private ImageSync CreateImageSync(uint imageIndex)
    {
        ...
        unsafe
        {
            var result = vk.CreateSemaphore(
                device,
                &semaphoreCreateInfo,
                null,
                out renderFinishedSemaphore
            );
            if (result != Result.Success)
            {
                throw new InvalidOperationException(
                    $"Failed to create render finished semaphore for image {imageIndex}: {result}"
                );
            }
        }

        return new ImageSync { RenderFinished = renderFinishedSemaphore, ImageIndex = imageIndex };
    }
    ...
    private FrameSync CreateFrameSync(int frameIndex)
    {
        ...
        unsafe
        {
            var result = vk.CreateSemaphore(
                ...
                &semaphoreCreateInfo,
                null,
                out imageAvailableSemaphore
            );
            if (result != Result.Success)
            {
                throw new InvalidOperationException(
                    $"Failed to create image available semaphore for frame {frameIndex}: {result}"
                );
            }
        }
        ...
        unsafe
        {
            ...
            if (result != Result.Success)
            {
                ...
                throw new InvalidOperationException(
                    $"Failed to create in-flight fence for frame {frameIndex}: {result}"
                );
            }
        }
