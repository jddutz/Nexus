...
public unsafe class DescriptorManager(IGraphicsContext context) : IDescriptorManager
{
    ...
    public GpuHandle CreateDescriptorSetLayout(DescriptorSetLayoutBinding[] bindings)
    {
        ...
        fixed (DescriptorSetLayoutBinding* pBindings = bindings)
        {
            ...
            var result = context.VulkanApi.CreateDescriptorSetLayout(
                context.Device,
                &layoutInfo,
                null,
                &layout
    ...
    public GpuHandle AllocateDescriptorSet(GpuHandle layout)
    {
        ...
        var result = context.VulkanApi.AllocateDescriptorSets(
            context.Device,
            &allocInfo,
            &descriptorSet
        );
        ...
        if (result == Result.ErrorOutOfPoolMemory || result == Result.ErrorFragmentedPool)
        {
            ...
            result = context.VulkanApi.AllocateDescriptorSets(
                context.Device,
                &allocInfo,
                &descriptorSet
            );
