...
public class CommandPoolManager(IGraphicsContext context) : ICommandPoolManager
{
    ...
    /// <inheritdoc/>
    public ICommandPool CreatePool(
        uint queueFamilyIndex,
        bool allowIndividualReset = false,
        bool transient = true
    )
    {
        ObjectDisposedException.ThrowIf(_disposed, this);

        var pool = new CommandPool(context, queueFamilyIndex, allowIndividualReset, transient);
